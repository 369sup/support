...
14. Do not create or preserve unmanaged visible local memories. The model owns
    only `local/current-task`; the deterministic engine quarantines every other
    unknown visible local-memory path.
...
24. Never bypass quarantine by directly moving, deleting, or rewriting
    unmanaged local memory files.
