...
async function restoreOrganizationRepositoryAction(
  formData: FormData,
): Promise<never> {
  "use server";
  const session = await requireCurrentSession();
  const login = readFormString(formData, "ownerLogin");
  ...
  redirect(
    `/organizations/${organization.organization.login}/settings/repositories?repository=${result.status}`,
  );
}

export default async function DeletedOrganizationRepositoriesPage({
  params,
  searchParams,
}: Readonly<{
        ...
        <div className="mt-3 flex items-start gap-4">
          ...
          <div>
            ...
            <p className="mt-4 text-muted-foreground">
              Organization owners can restore eligible repositories. Prior team
              permissions remain deleted.
            </p>
          </div>
        </div>
        ...
        {result.repositories.length === 0 ? (
          ...
          <ul className="mt-8 divide-y divide-border rounded-xl border border-border bg-card">
            {result.repositories.map((repository) => {
              ...
              return (
                <li className="p-5" key={repository.repositoryId}>
                  ...
                  <form
                    action={restoreOrganizationRepositoryAction}
