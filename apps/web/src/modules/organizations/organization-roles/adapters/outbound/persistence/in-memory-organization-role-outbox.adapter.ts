...
export interface InMemoryOrganizationRoleOutboxState {
  recordsByEventId: Map<string, OutboxRecord>;
}

type OrganizationRoleOutboxDependencies = Readonly<{
  nextEventId: () => string;
  now: () => string;
}>;

declare global {
  var __supportOrganizationRoleOutboxStateV2:
    | InMemoryOrganizationRoleOutboxState
    | undefined;
}

function createState(): InMemoryOrganizationRoleOutboxState {
  return { recordsByEventId: new Map() };
}

function getProcessState() {
  globalThis.__supportOrganizationRoleOutboxStateV2 ??= createState();
  return globalThis.__supportOrganizationRoleOutboxStateV2;
}

const systemDependencies: OrganizationRoleOutboxDependencies = {
  nextEventId: () => `event_${randomUUID()}`,
  now: () => new Date().toISOString(),
};

export class InMemoryOrganizationRoleOutboxAdapter
  implements EventRecorderPort, CommittedEventSourcePort
{
  readonly sourceId = "organizations/organization-roles";
  private readonly state: InMemoryOrganizationRoleOutboxState;
  private readonly dependencies: OrganizationRoleOutboxDependencies;
  ...
  constructor(
    state: InMemoryOrganizationRoleOutboxState = getProcessState(),
    dependencies: OrganizationRoleOutboxDependencies = systemDependencies,
  ) {
    this.state = state;
