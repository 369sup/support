    ...
    <main className="flex flex-1 flex-col bg-[#0d1117] px-5 text-slate-100">
      <section className="mx-auto flex w-full max-w-[360px] flex-1 flex-col justify-center py-12">
        ...
        <form action={googleSignInAction} className="mt-8">
          <Button
            className="h-11 w-full border-slate-600 bg-white font-semibold text-slate-950 hover:bg-slate-100 focus-visible:ring-emerald-400"
            disabled={!isEnabled}
            type="submit"
            variant="outline"
          >
        ...
        >
          ...
          <p>
            {isEnabled
              ? "Authentication is protected by the configured identity provider."
              : "Authentication is unavailable until this deployment is configured."}
          </p>
        </div>
      </section>
