...
| DB-EX-005 | Accepted | Legacy custom migration ledger | `public.support_schema_migrations` was intentionally excluded from declarative SQL during expand-contract rollout. The later forward migration drops only its obsolete checksum history after application contract and hosted route verification | `20260801144800_retire_legacy_migration_ledger.sql` | tracked-reference search, hosted login and route verification, `to_regclass('public.support_schema_migrations') is null`, and Supabase CLI ledger inspection |
