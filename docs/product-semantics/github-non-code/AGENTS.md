...
This file governs `docs/product-semantics/github-non-code/**` and adds only the
delta to [`docs/AGENTS.md`](../../AGENTS.md).
...
Do not load all diagrams by default. Stop once the affected actors, resources,
states, authorization layers, success and failure paths, side effects, and
unresolved variants are known.
...
Before using an atlas slice to implement or review product behavior, produce a
small acceptance checklist covering:

1. actors and resource ownership;
2. preconditions and independent state dimensions;
3. effective authorization inputs and denied cases;
4. success, conflict, and forbidden interaction paths;
5. notification, projection, moderation, and audit side effects where relevant;
6. unresolved plan, account, deployment, retention, or restoration variants.

Then map that checklist to the canonical architecture, active module catalog,
public entrypoints, API or command contracts, and tests. Do not infer missing
contracts from the Mermaid diagrams.
